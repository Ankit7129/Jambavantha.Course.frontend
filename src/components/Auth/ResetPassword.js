...
const ResetPassword = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      // Make sure to send 'newPassword' as per backend expectation
      const response = await axios.post(`${BASE_URL}/api/auth/reset-password/${token}`, {
        newPassword,
      });

      console.log(response.data); // Handle successful password reset here
      setSuccessMessage('Password reset successfully!');
      setErrorMessage('');

      // Redirect to login after successful reset
      setTimeout(() => {
        navigate('/login/student'); // Adjust based on userType if necessary
      }, 3000);
    } catch (error) {
